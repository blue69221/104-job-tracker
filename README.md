...
| 掃描 1,370 頁歷時 27 分鐘，期間公司刷新職缺會改變排序而漏掃 | 下架要求連續兩次沒看到（`CLOSE_GRACE_DAYS`），否則每天約 400 筆假下架與隔天假回鍋 |
| 一輪送出上千次資料庫請求，Supabase 連線偶爾被回收 | `Store` 全面重連重試；詳情補抓失敗不影響整輪結果 |
...
- `marks` 與 `blocked_companies` 沒有 `user_id` 欄位，是單人使用的設計。
  RLS policy 是「登入者皆可讀寫」，若日後要多人使用，必須加上使用者欄位並改寫 policy，
  否則所有人會共用同一份標記。
